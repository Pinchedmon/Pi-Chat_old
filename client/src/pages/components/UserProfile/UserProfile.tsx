...
function UserProfile() {
    ...
    useEffect(() => {
        getUserData(location.pathname.slice(1).toString()).then((res: {data: {0:IUser}}) => setUser(res.data[0]) )
    },[])
    return (
        ...
        <div>
            ...
            <div className='mt-16px'>
                {user.name !== '' && <CPost getPost={getMyPosts} naming='myPosts' getObject={user.name} />}
            </div>
        </div>
    );
}
