...
export const likeHandlerCom = (userId: string, id: number, likes: any) => {
  if (likes !== '0') {
    if (/\s/.test(likes)) {
      ...
      if (likes === userId) {
        axios.put(`http://localhost:6060/feed/comments?id=${id}&likes="0"`)
      } else {
        axios.put(`http://localhost:6060/feed/comments?id=${id}&likes='${likes + ` ${userId}`}' `)
      }
    }
  } else {
    axios.put(`http://localhost:6060/feed/comments?id=${id}&likes='${userId}'`)
  }
}
